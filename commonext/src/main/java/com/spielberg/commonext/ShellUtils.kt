...
fun execCmdAsync(
    ...
    return execCmdAsync(commands?.toArray<String>(arrayOf<String>()), isRooted, true, consumer)
}
...
fun execCmdAsync(
    ...
    return execCmdAsync(
        commands?.toArray<String>(arrayOf<String>()),
        isRooted,
        isNeedResultMsg,
        consumer
    )
...
fun execCmdAsync(
    ...
    isRooted: Boolean,
    isNeedResultMsg: Boolean,
    @NonNull consumer: Utils.Consumer<CommandResult?>?
): Utils.Task<CommandResult?>? {
    return UtilsBridge.doAsync(object : Task<CommandResult?>(consumer) {
        fun doInBackground(): CommandResult? {
...
fun execCmd(command: String, envp: List<String?>?, isRooted: Boolean): CommandResult? {
    return execCmd(
        arrayOf(command),
        envp?.toArray<String>(arrayOf<String>()),
        isRooted,
        true
    )
}
